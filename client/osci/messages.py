...
addMsg(Const.MSG_UPGRADED_SHIP, N_('Ship upgraded from %(1)s to %(2)s'), (type(str),type(str)), MIN)
addMsg(Const.MSG_DELETED_DESIGN, N_('Obsolete ship design deleted: %(1)s'), (type(str),), CRI)
addMsg(Const.MSG_CANNOT_UPGRADE_SR, N_('Cannot upgrade ship from %(1)s to %(2)s\n\nCannot upgrade ship from %(1)s to %(2)s because of we have not enough of %(3)s.'), (type(str),type(str),stratID2Name), MAJ)
...
addMsg(Const.MSG_ENTERED_WORMHOLE, N_('Your fleet entered a wormhole at %(1)s and exited at %(2)s.'), (type(str),type(str)), severity = MIN)
...
addMsg(Const.MSG_FUEL_LOST_ORBITING, N_('Fleet lost.\n\n We lost contact with the %(1)s after they ran out of fuel in the system %(2)s.'), (type(str), objID2Name), severity = MAJ)
addMsg(Const.MSG_FUEL_LOST_FLYING, N_('Fleet lost.\n\n We lost contact with the %(1)s after they ran out of fuel en route to the system %(2)s.'), (type(str), objID2Name), severity = MAJ)
...
def getMsgText(msgID, data):
    ...
    try:
        # tranform data
        newData = {}
        if not (type(data) == list or type(data) == TupleType):
            data = (data,)
        if transform:
            index = 1
